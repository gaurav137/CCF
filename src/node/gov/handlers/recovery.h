...
namespace ccf::gov::endpoints
{
    ...
    ccf::AbstractNodeContext& node_context)
  {
    ...
    auto get_multiple_encrypted_shares_for_member =
      [&](auto& ctx, ApiVersion api_version) {
        switch (api_version)
        {
          case ApiVersion::preview_v1:
          ...
          default:
          {
            ...
            auto encrypted_shares =
              ShareManager::get_multiple_encrypted_shares(ctx.tx, member_id);

            if (!encrypted_shares.has_value())
            {
              detail::set_gov_error(
                ctx.rpc_ctx,
                HTTP_STATUS_NOT_FOUND,
            ...
            std::vector<std::string> encoded_shares;
            for (auto &encrypted_share : encrypted_shares.value())
            {
              encoded_shares.emplace_back(ccf::crypto::b64_from_raw(encrypted_share));  
            }

            auto response_body = nlohmann::json::object();
            response_body["memberId"] = member_id;
            response_body["encryptedShares"] = encoded_shares;
    ...
    registry
      .make_read_only_endpoint(
        "/recovery/multiple-encrypted-shares/{memberId}",
        HTTP_GET,
        api_version_adapter(get_multiple_encrypted_shares_for_member),
        ccf::no_auth_required)
      .set_openapi_hidden(true)
      .install();

    auto submit_multiple_recovery_shares = [&](auto& ctx, ApiVersion api_version) {
      switch (api_version)
      {
        case ApiVersion::preview_v1:
        case ApiVersion::v1:
        default:
        {
          ...
          try
          {
            submitted_shares_count = share_manager.submit_multiple_recovery_shares(
              ctx.tx, member_id, raw_recovery_shares);

            for (auto& raw_recovery_share : raw_recovery_shares)
            {
              OPENSSL_cleanse(
                raw_recovery_share.data(), raw_recovery_share.size());
            }
