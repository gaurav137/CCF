...
def test_npm_app(network, args):
    ...
    with primary.client("user0") as c:
        ...
        r = c.post(
            ...
            {"privateKey": privk, "subjectName": "foo", "validityPeriodDays": 2},
        )
        assert r.status_code == http.HTTPStatus.OK, r.status_code
        r = c.post("/app/isValidX509CertBundle", r.body.json()["cert"])
        ...
        if args.enclave_platform != "sgx":
            ...
            r = c.post(
                "/app/verifySnpAttestation",
                {
                    "evidence": reference_quote["quote"],
